<#
.SYNOPSIS
    Antigravity CLI (agy) Universal Multi-Format Harness & Safety Rule Installer
.DESCRIPTION
    Injects agy CLI protection rules for PDF, DOCX, XLSX, PPTX, Images & Archives,
    autonomous execution protocols, and installs the agy-safe wrapper for all agy users on Windows.
#>
...
Write-Host " Antigravity CLI (agy) Universal Multi-Format Harness Setup " -ForegroundColor White
Write-Host "==========================================================" -ForegroundColor Cyan
Write-Host ""
...
- **全自主檔案搬運與複製**：遇到 NAS/網路磁碟 (`X:\`)、中文或含空格路徑時，Agent 必須**自主使用 PowerShell (`Copy-Item -LiteralPath ...`)** 複製到本地短路徑（如 `C:\temp\`），**嚴禁要求用戶手動搬運檔案**。
...
function Patch-ConfigFile {
    ...
    if (Test-Path -LiteralPath $FilePath) {
        ...
        if ($content -notlike "*agy CLI 全格式二進位檔與文檔處理鐵律*") {
            Add-Content -LiteralPath $FilePath -Value $ruleBlock -Encoding UTF8
            Write-Host "[+] 成功注入 agy 全格式安全鐵律至: $FilePath" -ForegroundColor Green
        } else {
            Write-Host "[~] 檔案已包含 agy 全格式安全鐵律: $FilePath" -ForegroundColor Yellow
        }
    } else {
        Set-Content -LiteralPath $FilePath -Value ("# 全域指南`n" + $ruleBlock) -Encoding UTF8
        Write-Host "[+] 建立並寫入 agy 全格式安全鐵律至: $FilePath" -ForegroundColor Green
    }
}
...
Write-Host ""
Write-Host "==========================================================" -ForegroundColor Cyan
Write-Host "   安裝完成！已支援 PDF/DOCX/XLSX/PPTX/JPG 通用自動防禦 " -ForegroundColor Green
Write-Host "==========================================================" -ForegroundColor Cyan
